...
configFile = installDir + "/orionkit.cfg"
print(installDir)
config = ConfigParser.RawConfigParser()
config.read(configFile)

toolDir = installDir + config.get('orionkit', 'toolDir')
logDir = installDir + config.get('orionkit', 'logDir')
yes = config.get('orionkit', 'yes').split()
color_random=[color.HEADER,color.IMPORTANT,color.NOTICE,color.OKBLUE,color.OKGREEN,color.WARNING,color.RED,color.END,color.UNDERLINE,color.LOGGING]
random.shuffle(color_random)
orionkitlogo = color_random[0] + '''
...
╚██████╔╝██║  ██║██║╚██████╔╝██║ ╚████║██║  ██╗██║   ██║
 ╚═════╝ ╚═╝  ╚═╝╚═╝ ╚═════╝ ╚═╝  ╚═══╝╚═╝  ╚═╝╚═╝   ╚═╝
        ...
        '''
orionkitPrompt = "orionkit ~# "
alreadyInstalled = "Already Installed"
continuePrompt = "\nClick [Return] to continue"

termsAndConditions = color.NOTICE + '''
I shall not use orionkit to:
...
'''
def agreement():
    while not config.getboolean("orionkit", "agreement"):
        clearScr()
        print(termsAndConditions)
        agree = raw_input("You must agree to our terms and conditions first (Y/n) ").lower()
        if agree in yes:
            config.set('orionkit', 'agreement', 'true')

class orionkit:
    def __init__(self):
        clearScr()
        self.createFolders()
        print (orionkitlogo + color.RED + '''
       }--------------{+} Coded By Orion {+}--------------{
       }--------{+}  github.com/01r0n/orionkit {+}--------{
    ''' + color.END + '''
       ...
       {99}-EXIT\n
     ''')
        choice = raw_input(orionkitPrompt)
        clearScr()
        if choice == "1":
    ...
    Yb      Yb   dP 88 Y88   88   88"Yb  88 88""Yb o.`Y8b
     ...
     ''')
        contributorsURL = 'https://api.github.com/repos/Orion/orionkit/contributors'
    ...
    def update(self):
        os.system("git clone --depth=1 https://github.com/01r0n/orionkit.git")
        os.system("cd orionkit && bash ./update.sh")
        os.system("orionkit")
...
class sniffingSpoofingMenu:
    ...
    def __init__(self):
        ...
        choice6 = raw_input(orionkitPrompt)
        clearScr()
        if choice6 == "1":
            setoolkit()
        elif choice6 == "2":
...
class webHackingMenu:
    ...
    def __init__(self):
        ...
        print("   {99}-Back To Main Menu \n")
        choiceweb = raw_input(orionkitPrompt)
        clearScr()
        if choiceweb == "1":
            maine()
        elif choiceweb == "2":
...
class postExploitationMenu:
    ...
    def __init__(self):
        ...
        choice11 = raw_input(orionkitPrompt)
        clearScr()
        if choice11 == "1":
            sitechecker()
        elif choice11 == "2":
...
class informationGatheringMenu:
    ...
    def __init__(self):
        ...
        print("  {8}--Crips\n  ")
        print("  {99}-Back To Main Menu \n")
        choice2 = raw_input(orionkitPrompt)
        clearScr()
        if choice2 == "1":
            nmap()
        elif choice2 == "2":
...
class crips:
    ...
    def __init(self):
        ...
        self.gitRepo = "https://github.com/01r0n/Crips.git"
...
class wirelessTestingMenu:
    ...
    def __init__(self):
        ...
        choice4 = raw_input(orionkitPrompt)
        clearScr()
        if choice4 == "1":
            reaver()
        elif choice4 == "2":
...
class exploitationToolsMenu:
    ...
    def __init__(self):
        ...
        choice5 = raw_input(orionkitPrompt)
        clearScr()
        if choice5 == "1":
            atscan()
        elif choice5 == "2":
